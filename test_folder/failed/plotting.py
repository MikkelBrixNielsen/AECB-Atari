...
from utils import VALUE_CONTAINER, log, sample_memory
...
def plot_codebook_usage(model, memory, log_dir, epoch, seed, batch_size=5000, usage_log=None):
    ...
    for s, _, sp, _, _ in zip(*sample_memory(memory, min_size)):
    ...
    log(log_dir, f"\t" + codebook_usage, console_log=VALUE_CONTAINER.debug_mode, no_log=True)
...
def plot_N_sa_histogram(N_sa, log_dir, epoch, seed):
    ...
    plt.hist(N_sa, bins=30, log=True)
