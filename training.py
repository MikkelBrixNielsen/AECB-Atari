...
def estimate_codebook_usage_probs(model, x): # ChatGPT Suggestion
    z_e = model.encoder(x)
...
def entropy_bonus(probs, scale=0.05): # ChatGPT Suggestion
    return -scale * (-torch.sum(probs * torch.log(probs + 1e-8)))

def usage_penalty(model, probs, scale=0.05): # ChatGPT Suggestion
